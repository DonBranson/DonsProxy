...
public class HttpHeadSubscriberTest {
	...
	public void testGetRequest() throws Exception {
		...
		int expectedLength = request.getBytes().length + line0.length() + line1.length() + line2.length() + lineSeparator.length() * 3; 
	...
	public void testConnectRequest() throws Exception {
		...
		int expectedLength = requestHeaders.getBytes().length + line0.length() + line1.length() + line2.length() + lineSeparator.length() * 3; 
		Assert.assertEquals(expectedLength, baos.size());
	}
	...
	public void testAllHeaderVariants() throws Exception {
		...
		for (int line = 0; line < knownHeaders.length; line++) {
			...
			expectedLength += line1.length() + line2.length() + lineSeparator.length() * 3; 
	...
	public void testAllNonHeaderVariants() throws Exception {
		...
		for (int line = 0; line < notHeaders.length; line++) {
			...
			BufferedReader reader = new BufferedReader(new StringReader(baos.toString()));
			String line0 = reader.readLine();
			Assert.assertEquals("Non-header variant " + request, line0.length() + lineSeparator.length(), baos.size());
		}
	}
	...
	private ByteArrayOutputStream startSubscriber() throws InterruptedException {
		...
		Thread subscriberThread = new Thread(){
			...
			public void run() {
				...
				try {
					new HttpHeadSubscriber().start(settings, baos);
				} catch (IOException e) {
