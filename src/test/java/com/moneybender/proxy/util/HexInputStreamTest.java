...
public class HexInputStreamTest {
	...
	public void testConversion() throws Exception {
		...
		HexInputStream hexInputStream =
			new HexInputStream(new ByteArrayInputStream(stringWriter.getBuffer().toString().getBytes()));
		
		byte[] buffer = new byte[sampleString.getBytes().length];
		new DataInputStream(hexInputStream).readFully(buffer);
